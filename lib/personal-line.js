...
const PUBLIC_SAMPLE_LINE = {
  ...
  anchor: "\uC624\uB298\uC758 \uBC29\uD5A5",
  keywordsInput:
    "\uBB3C \uB9C8\uC2DC\uAE30, \uC9E7\uAC8C \uAC77\uAE30, \uC77D\uAE30 10\uBD84, \uCC45\uC0C1 \uC815\uB9AC, \uB0B4\uC77C \uCCAB \uC791\uC5C5 \uC815\uD558\uAE30",
  chips: [
    "\uC624\uB298\uC758 \uBC29\uD5A5",
    "\uBB3C \uB9C8\uC2DC\uAE30",
    "\uC9E7\uAC8C \uAC77\uAE30",
...
function getPersonalSeedLine() {
  const seedLine = normalizeLine(getLine("default"));
  if (!seedLine) return { ...EMPTY_LINE, createdAt: Date.now(), updatedAt: Date.now() };
  return {
    ...seedLine,
    id: "default",
    createdAt: Date.now(),
...
export async function getLineForSession(session) {
  ...
  if (!canUsePostgres()) {
    const existing = getLine(`user:${key}`);
    if (existing) return existing;

    const seeded = getPersonalSeedLine();
    return saveLine({ ...seeded, id: `user:${key}` });
  }
  ...
  const seeded = getPersonalSeedLine();
  await saveLineForSession(session, seeded);
  return seeded;
}
