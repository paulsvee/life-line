  ...
  if (!existing) {
    ...
    if (seed) {
      ...
      `).run(
        ...
        seed.anchor,
        seed.keywordsInput,
        JSON.stringify(seed.chips),
        JSON.stringify(seed.colors),
        seed.createdAt,
        seed.updatedAt
      );
    } else {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `).run("default", "", "", JSON.stringify([""]), JSON.stringify([null]));
    }
  } else {
    const currentDefault = db.prepare(`
      SELECT anchor, chips_json, colors_json
      FROM life_lines
      WHERE id = ?
    `).get("default");

    if (currentDefault?.anchor === "나는") {
      const chips = JSON.parse(currentDefault.chips_json);
      const nextChips = Array.isArray(chips) && chips.length > 0 ? ["", ...chips.slice(1)] : [""];
      db.prepare(`
        UPDATE life_lines
        SET anchor = ?, chips_json = ?, colors_json = ?, updated_at = ?
        WHERE id = ?
      `).run("", JSON.stringify(nextChips), JSON.stringify(new Array(nextChips.length).fill(null)), Date.now(), "default");
    }
  }
